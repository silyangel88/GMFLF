...
class EFSM(object):
    ...
    def get_init_state(self):
        if self.states_table:
            return self.states_table[0]
        else:
            return 's1'
    ...
    def is_feasible(self, cur_transition, sc=None):
        ...
        try:
            ...
            print(cur_transition.get_trans_name())
            print(e)
            return False
    ...
    def update_transition_output(self, cur_transition, contexts, input_params):
        ...
        params_pattern = re.findall(p1, output_event)
        if params_pattern is None:
            return output_event

        result_event = output_event
        for params in params_pattern:
            param = params.split(',')
            for idx, p in enumerate(param):
                p = p.strip()
                if p in contexts:
                    param[idx] = str(contexts.get(p))
                elif p in input_params:
                    param[idx] = str(input_params.get(p))
            tmp_params = ','.join(param)
            result_event = result_event.replace(params, tmp_params, 1)
        ...
        if transition:
            ...
            if guard and guard != '':
                ...
                try:
                    ...
                    print(transition.get_trans_name())
                    print(e)
                    return False
        return content
