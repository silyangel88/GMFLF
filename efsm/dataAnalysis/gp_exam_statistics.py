...
if __name__ == "__main__":
    ...
    '''读取scoreFiles文件'''
    scorefiles = all_path(r'D:\pythonProject\efsm\dataAnalysis\RM2(250)\KWOA_LM')     # !!!
    root = r'D:\pythonProject\efsm\dataAnalysis\RM2(250)\KWOA_LM'                     # !!!

    formulaDict = initFormulaDict()
    formulasList = list(formulaDict.keys())  # 获取公式名
    ...
    for k in range(30):
        ...
        for i in range(len(file_paths)):
            ...
            formulas_list = list(gpFormulaDict.values())                 # 字典值转列表——导出公式
            Ranktable, EXAMtable, evaluation_Table, time2 = GPformulas(formulas_list, formulaset, spectrum, indexList[i],
                                                Ranktable, EXAMtable, evaluation_Table, total_transitions, faultpos)
        ...
        for i in range(len(scoreFiles)):
            ...
            Ranktable, EXAMtable, evaluation_Table, time3 = combineFormulas(scoredata, indexList[i], Ranktable, EXAMtable, evaluation_Table, total_transitions, int(faultPos))
